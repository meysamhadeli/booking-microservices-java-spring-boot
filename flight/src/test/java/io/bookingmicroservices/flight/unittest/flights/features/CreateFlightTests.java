...
public class CreateFlightTests extends UnitTestBase {
  ...
  public void should_throw_exception_when_flight_is_already_exist() {
    ...
    verify(flightRepository, times(0)).create(any(FlightEntity.class)); // Verify that create is not called
  }
  ...
  public void should_return_valid_flight_dto_when_we_create_a_flight() {
    ...
    when(flightRepository.create(any(FlightEntity.class))).thenReturn(FlightEntityFake.generate());
    ...
    verify(flightRepository, times(1)).create(any(FlightEntity.class)); // Verify that create was called
  }
}
